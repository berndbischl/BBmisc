prettyPrint = function(x, indent.long = NULL, prefix = "", sep = ": ", key.align = "left", width = getOption("width") - 1L) {
  ...
  nc.right = width - nc.left
  nc.indent = ifelse(is.null(indent.long), nc.key + nc.sep, asCount(indent.long))
  ...
  for (i in seq_along(x)) {
    ...
    if (is.atomic(val)) {
      ...
      val = paste0("\n", collapse(paste0(prefix, strrepeat(" ", nc.indent), capture.output(print(val))), "\n"))
    }
